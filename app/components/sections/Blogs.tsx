          ...
          <div className="flex items-center justify-start mx-4 lg:mx-10 gap-10 h-full w-full overflow-x-scroll scrollbar-hide">
            {BLOGS.map((blog) => (
              <Link
                key={blog.id}
                href={blog.link}
                className="h-full flex flex-col items-start justify-between gap-5 group flex-shrink-0 min-w-[300px] lg:min-w-[400px]"
              >
                ...
                <h2 className={`text-2xl ${bagel.className}`}>{blog.text}</h2>
              </Link>
            ))}
          </div>
        </div>
